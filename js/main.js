/**
*  Main function.
*  Is called when user clicks on "GO" button.
*  Check the input, make corresponding calls, 
*  and print outcome to website. 
**/
function main() {
    ...
    switch (method) {
        case "1": printResult(otpEncrypt(message, key));
            break;
        case "2": printResult(otpDecrypt(message, key));
            break;
...
/**
*   One-time-pad decryptor.
*   Decrypt given message using the key with one time pad method.
**/
function otpDecrypt(message, key) {
    if (!key) {
        alert("Key required to decrypt");
        return;
    }
    var decryptedMessage = "";
    var msgInd, keyInd, index;
    ...
    for (var i = 0; i < message.length; i++) {
        if (isLetter(message[i])) {
            try {
            msgInd = alpha.indexOf(message[i].toLowerCase());
            keyInd = alpha.indexOf(key[i].toLowerCase());
            index = (msgInd - keyInd);        
            if (index < 0) { index += 26; }
            if (message[i] === message[i].toUpperCase()) {
                decryptedMessage += alphaUpper.charAt(index % alphalength);
            } else {
                decryptedMessage += alpha.charAt(index % alphalength);
            }
            } catch(Error) {
...
*   Encrypt the message using the given key and one time pad method.
*   If key is not given auto generate it.
**/
function otpEncrypt(message, key) {
    if (!key) { 
        key = generateKey(message);
        alert("Auto-generated Key: " + key);
    }
    ...
    for (var i = 0; i < message.length; i++) {
        if (isLetter(message[i])) {
            try {
                ...
                index = (msgInd + keyInd);
                if (message[i] === message[i].toUpperCase()) {
                    encryptedMessage += alphaUpper.charAt(index % alphalength);
                } else {
                    encryptedMessage += alpha.charAt(index % alphalength);
                }
            } catch(Error) {
...
/**
*   Auto generate key for one time pad encryptor.
**/
function generateKey(message){
    var key = '';
    var alpha = "abcdefghijklmnopqrstuvwxyz";
    for(var i = 0; i < message.length; i++)
        key += alpha.charAt((Math.floor(Math.random() * 26) + 65) % 26);
    return key;
}

/**
*   Print the given message to the website.
**/
...
/**
 * Create a SHA256 hash of the given message
 * 
...
function SHA256(s){
 ...
 function core_sha256 (m, l) {
   var K = new Array(0x428A2F98, 0x71374491,
